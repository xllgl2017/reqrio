...
async fn test_tls12() {
    ...
    //h2
    let mut req = AcReq::new().with_timeout(Timeout::longer()).with_alpn(ALPN::HTTP20);
...
async fn test_auto_redirect() {
    let mut req = AcReq::new().with_timeout(Timeout::longer()).with_alpn(ALPN::HTTP20).with_auto_redirect(false);
