...
impl Cipher {
    ...
    fn build_aad(&self, layer: &RecordLayer, encrypt: bool) -> HlsResult<[u8; 13]> {
        let mut res = [0; 13];
        ...
        let payload = layer.message.payload().ok_or(HlsError::NonePointer)?;
    ...
    pub fn encrypt(&mut self, record: &mut RecordLayer) -> HlsResult<()> {
        let add_arr = self.build_aad(&record, true)?;
        let aad = Aad::from(&add_arr);
        let encrypter = self.key.encrypter().ok_or(HlsError::EncrypterNone)?;
        let nonce = Nonce::assume_unique_for_key(self.iv.as_array(self.seq));
        let payload = record.message.payload_mut().ok_or(HlsError::PayloadNone)?;
    ...
    pub fn decrypt(&mut self, mut record: RecordLayer) -> HlsResult<Vec<u8>> {
        let add_arr = self.build_aad(&record, false)?;
        let aad = Aad::from(&add_arr);
        let decrypter = self.key.decrypter().ok_or(HlsError::DecrypterNone)?;
        let mut payload = record.message.take_payload().ok_or(HlsError::PayloadNone)?.to_bytes();
