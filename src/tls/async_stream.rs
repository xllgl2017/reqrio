...
impl<S: AsyncRead + AsyncWrite + Unpin> AsyncStream<S> {
    pub async fn connect(mut param: ConnParam<'_>, mut stream: S) -> HlsResult<AsyncStream<S>> {
        ...
        stream.write(&client_hello.as_bytes()).await?;
        stream.flush().await?;
        ...
        while !stream.handshake_finished {
            let record = stream.read_packet().await?;
            stream.handle_message(record, &mut param).await?;
        }
        let record = stream.read_packet().await?;
    ...
    pub async fn read_packet(&mut self) -> HlsResult<RecordLayer> {
        let mut head = [0; 5];
        let len = self.stream.read(&mut head).await?;
        ...
        buffer.resize(res.len as usize, 0);
        let mut index = 0;
        while index < res.len as usize {
            let len = self.stream.read(&mut buffer[index..]).await?;
            if len == 0 { return Err(HlsError::PeerClosedConnection); }
            index += len;
        }
    ...
    async fn handle_message(&mut self, record: RecordLayer, param: &mut ConnParam<'_>) -> HlsResult<()> {
            ...
            RecordType::HandShake => {
                match record.message {
                    ...
                    Message::ServerHelloDone(_) => {
                        ...
                        self.stream.write(&client_key_exchange.as_bytes()).await?;
                        self.stream.flush().await?;

                        self.stream.write(&param.fingerprint.change_cipher_spec.as_bytes()).await?;
                        self.stream.flush().await?;
                        ...
                        self.stream.write(&record_layer.as_bytes()).await?;
                        self.stream.flush().await?;
                    }
                    _ => {}
                }
    ...
    pub async fn read_tls(&mut self) -> HlsResult<Vec<u8>> {
        let layer = self.read_packet().await?;
        let payload = self.conn.read_message(layer)?;
        Ok(payload)
    }

    pub async fn write_tls(&mut self, buf: &[u8]) -> HlsResult<()> {
        let layer = self.conn.make_message(RecordType::ApplicationData, buf.to_vec())?;
        self.stream.write(&layer.as_bytes()).await?;
